...
%numerical solver (this is for Amensalism)

function init=input_initial_general_for_arg_6(default)

%Define initial conditions (A(0),a(0), B(0), b(0), C(0))
...
C_init_in=input('Enter initial value of C(>0):');
    if C_init_in>=0
        C_init=C_init_in;
    else
        C_init=0.1; %Default value
        msgbox('The value entered for the initial value of C was not valid so default value was selected','Error','error')
    end
init=[A_init, a_init, B_init, b_init, C_init];
end